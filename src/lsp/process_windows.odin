...
// Stdio pipes connecting us to the language-server child. The "_read"
// handles are what we *read from* (server's stdout/stderr); "_write" is
// what we *write to* (server's stdin). The child-side ends are closed in
// `process_spawn` once CreateProcessW has duplicated them.
ProcessState :: struct {
...
process_spawn :: proc(state: ^ProcessState, command_tokens: []string, working_directory: string = "") -> bool {
	...
	// Mark *our* sides of the pipes as non-inheritable so they aren't
	// duplicated into the child along with the inherited stdio handles.
	...
	process_created := win32.CreateProcessW(
		...
		nil, nil,
		true, // bInheritHandles — necessary for the child to see our pipe ends
	...
	// Whether the spawn succeeded or not, the *child-side* pipe ends are
	// either now owned by the child (success) or no longer useful (failure).
...
// Phase 1 of shutdown — terminate the child and close the read pipe so the
// reader thread's PeekNamedPipe loop notices and exits. Symmetric with the
// terminal's two-phase shutdown.
@(private)
...
// Non-blocking read: returns 0 bytes (with ok=true) when the pipe is idle so
// the reader thread can re-check `is_alive` instead of parking forever.
// Returns ok=false when the pipe is broken / closed.
@(private)
...
// Quote tokens that contain whitespace; concatenate with spaces. Caller owns
// nothing — result lives in temp_allocator.
@(private="file")
