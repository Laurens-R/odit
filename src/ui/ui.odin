...
draw_filled_rounded_rect :: proc(renderer: ^sdl3.Renderer, rectangle: sdl3.FRect, radius: f32, color: sdl3.FColor) {
	...
	for y_offset_int in 0..<i32(r) {
		...
		bottom := sdl3.FRect{ rectangle.x + x_inset, rectangle.y + rectangle.h - 1 - y_offset, row_width, 1 }
...
draw_rounded_rect_outline :: proc(renderer: ^sdl3.Renderer, rectangle: sdl3.FRect, radius: f32, color: sdl3.FColor) {
	sdl3.SetRenderDrawColorFloat(renderer, color.r, color.g, color.b, color.a)
	r := radius
	if r <= 0 || rectangle.w < 2 * r || rectangle.h < 2 * r {
		rect := rectangle
	...
	rounded_rect_corner_arc(renderer, right_x - r,               rectangle.y + r,           r, 1.5 * math.PI,     2.0 * math.PI)
...
@(private="file")
rounded_rect_corner_arc :: proc(renderer: ^sdl3.Renderer, center_x, center_y, radius, start_angle, end_angle: f32) {
	segments :: 8
...
scroll_view_begin :: proc(ui_context: ^Context, scrollbar: ^Scrollbar, viewport: sdl3.FRect, scroll_value: ^i32, content_height: i32) -> (origin_x, origin_y: i32, scroll_view: ScrollView) {
...
scroll_view_end :: proc(scroll_view: ScrollView, theme: Theme) {
	...
	right_edge_x := i32(scroll_view.viewport.x + scroll_view.viewport.w) + 2 + SCROLLBAR_NARROW_WIDTH
	scrollbar_render(
...
scrollbar_thumb_hit :: proc(scrollbar: ^Scrollbar, mouse_x, mouse_y: f32) -> bool {
	...
	return mouse_x >= thumb.x - SCROLLBAR_THUMB_HIT_PAD && mouse_x < thumb.x + thumb.w + SCROLLBAR_THUMB_HIT_PAD &&
	       mouse_y >= thumb.y                            && mouse_y < thumb.y + thumb.h
}
...
draw_button :: proc(ui_context: ^Context, button_rectangle: sdl3.FRect, label: string, is_focused: bool, theme: Theme) {
	...
	if is_focused      { background_color = theme.title_background }
	else if is_hovered { background_color = theme.hover_background }
	...
	if is_focused {
		inner_border := sdl3.FRect{button_rectangle.x + 1, button_rectangle.y + 1, button_rectangle.w - 2, button_rectangle.h - 2}
