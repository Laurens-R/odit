// Input event handling + render for the terminal picker.
...
// Per-event dispatch. Returns `(Intent, needs_redraw)`; callers that
// don't want manual intent routing should use `dispatch_event` in
// `dispatch.odin`.
handle_event :: proc(state: ^State, event: ^sdl3.Event) -> (intent: Intent, needs_redraw: bool) {
	...
	case .KEY_DOWN:
		...
		// Ctrl+Shift+F9 and plain F9 from inside the picker close —
		// mirrors how F4 closes the open-docs dialog.
...
render :: proc(state: ^State, ui_context: ^ui.Context, viewport_width, viewport_height: i32) {
	...
	if len(state.filtered_indices) == 0 {
		...
		for row_index := state.scroll_offset; row_index < end_row_index; row_index += 1 {
			...
			if entry_index < 0 || entry_index >= len(state.entries) { continue }
